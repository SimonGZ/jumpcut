...
fn paginator_distributes_blocks_across_pages_when_they_exceed_limits() {
    ...
    assert_eq!(
        pages[1].blocks.len(),
        1,
        "Second block rolls cleanly to Page 2"
    );
}
...
fn paginator_strips_intrinsic_padding_from_elements_landing_at_the_top_of_a_page() {
    ...
    assert_eq!(
        pages[1].blocks[0].spacing_above, 0.0,
        "Top padding must be stripped to 0 at page boundaries"
    );
...
fn paginator_ignores_page_start_markers_when_stripping_top_of_page_spacing() {
    let geometry = LayoutGeometry::default();
    let page_limit = 54.0;
    let page_start = SemanticUnit::PageStart(PageStartUnit {
        source_element_id: "page-start".into(),
    });
...
fn paginator_starts_a_new_page_when_it_encounters_a_page_start_marker() {
    ...
    assert_eq!(
        pages.len(),
        2,
        "PageStart should force a new page even when plenty of space remains"
    );
    assert_eq!(
        pages[0].blocks.len(),
        1,
        "The visible content before the PageStart should stay on page 1"
    );
    assert_eq!(
        pages[1].blocks.len(),
        2,
        "Page 2 should contain the PageStart marker and the following visible block"
    );
    assert_eq!(
        pages[1].blocks[1].spacing_above, 0.0,
        "The first visible block after a forced page break should strip intrinsic spacing"
    );
}
...
fn paginator_prevents_stranding_blocks_that_require_keep_with_next() {
    ...
    assert_eq!(
        pages[0].blocks.len(),
        1,
        "Page 1 should only contain the first block because the Scene Heading was pushed"
    );
    ...
    assert_eq!(
        pages[1].blocks.len(),
        2,
        "Page 2 should contain the Scene Heading and the Action block"
    );

    // Top-of-page rule applies to the pushed Scene Heading!
    assert_eq!(
        pages[1].blocks[0].spacing_above, 0.0,
        "Scene Heading was pushed to the top of Page 2, so padding is stripped"
    );
...
fn paginator_splits_splittable_blocks_while_respecting_orphan_widow_limits() {
    ...
    assert_eq!(
        pages[0].blocks.len(),
        2,
        "Page 1 contains the 51-line block and the top half of the split block"
    );
    assert_eq!(
        pages[0].blocks[1].content_lines, 2.0,
        "2 lines of the split block fit on Page 1 (Orphan limit respected)"
    );

    assert_eq!(
        pages[1].blocks.len(),
        1,
        "Page 2 gets the bottom half of the split block"
    );
    assert_eq!(
        pages[1].blocks[0].spacing_above, 0.0,
        "Top padding stripped for the widow piece"
    );
    assert_eq!(
        pages[1].blocks[0].content_lines, 3.0,
        "Remaining 3 lines push to Page 2"
    );
}
...
fn paginator_rejects_splits_that_violate_orphan_limits_and_pushes_entire_block() {
    ...
    assert_eq!(
        pages[0].blocks.len(),
        1,
        "Page 1 holds the 53-line block, leaves the 1 remaining line blank"
    );

    assert_eq!(
        pages[1].blocks.len(),
        1,
        "The entire 4-line block is pushed to Page 2 to avoid a 1-line orphan"
    );
...
fn paginator_accounts_for_additional_widow_penalty_lines_when_splitting_dialogue() {
    ...
    assert_eq!(pages[0].blocks.len(), 2);
    assert_eq!(
        pages[0].blocks[1].content_lines, 3.0,
        "3 lines of the split dialogue fit on Page 1"
    );

    // The widow block on Page 2 receives the structural penalty
    assert_eq!(
        pages[1].blocks[0].content_lines, 5.0,
        "Remaining 4 lines + 1 penalty = 5 lines on Page 2"
    );
}
...
fn paginator_respects_custom_orphan_widow_limits() {
    ...
    assert_eq!(
        pages[0].blocks.len(),
        2,
        "Should split exactly at the 4-line orphan limit"
    );
    ...
    assert_eq!(
        pages2[0].blocks.len(),
        1,
        "Should NOT split if available space (3.0) < orphan limit (4.0)"
    );
}
...
fn paginator_verifies_final_draft_parity_for_1_5_leading() {
    ...
    let units: Vec<SemanticUnit> = (0..36)
        .map(|i| visible_unit(&format!("el-{}", i)))
        .collect();
    ...
    assert_eq!(
        pages.len(),
        1,
        "36 lines @ 1.5 leading (54.0 total) must fit on a single 54-line page"
    );
    assert_eq!(
        pages[0].blocks.len(),
        36,
        "All 36 blocks should be on Page 1"
    );
}
...
fn paginator_verifies_final_draft_parity_for_2_0_leading() {
    ...
    // They should fit EXACTLY on one 54.0-line page without a spill.
    let mut blocks = Vec::new();
    let units: Vec<SemanticUnit> = (0..27)
        .map(|i| visible_unit(&format!("el-{}", i)))
        .collect();
    ...
    assert_eq!(
        pages.len(),
        1,
        "27 lines @ 2.0 leading (54.0 total) must fit on a single 54-line page"
    );
    assert_eq!(
        pages[0].blocks.len(),
        27,
        "All 27 blocks should be on Page 1"
    );
}
