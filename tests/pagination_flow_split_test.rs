...
fn flow_split_prefers_a_sentence_boundary_when_a_legal_split_exists() {
    ...
    assert_eq!(
        ...
        "A forty-year old man named BEAMEN comes out of the seed store to greet Edward."
    );
    ...
    assert!(decision
        .bottom_text
        .trim_start()
        .starts_with("Friendly but a little drunk"));
}
...
fn flow_split_allows_exact_fit_sentence_fragment_even_with_a_short_last_line() {
    ...
    let decision = choose_flow_split_allow_exact_fit_sentence_runt(text, &config, 2, 2, 2).unwrap();
